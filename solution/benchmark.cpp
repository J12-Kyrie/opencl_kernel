...
// ---- CPU Reference: Bilinear Resize + Normalize (float data) ----
static void cpuBaseline(const std::vector<float>& src, int sw, int sh,
    ...
    for (int dy = 0; dy < dh; dy++) {
        for (int dx = 0; dx < dw; dx++) {
            ...
            int s4 = sw * 4;
            int o00 = iy0*s4 + ix0*4, o01 = iy0*s4 + ix1*4;
            int o10 = iy1*s4 + ix0*4, o11 = iy1*s4 + ix1*4;
            // src values are already in [0,1] (float)
            ...
            int out = (dy*dw + dx)*3;
            dst[out]   = (r - MEAN_R) / STD_R;
            dst[out+1] = (g - MEAN_G) / STD_G;
            dst[out+2] = (b - MEAN_B) / STD_B;
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    for (int ri : res_idx) {
        ...
        int sstride = sw * 4 * (int)sizeof(float);  // RGBA float stride
        size_t src_bytes = (size_t)sh * sstride;
        size_t norm_bytes = (size_t)DST_H * DST_W * CHANNELS * sizeof(float);

        // Generate float RGBA image data (pre-normalized [0,1] for CL_FLOAT format)
        std::vector<float> src_data((size_t)sh * sw * 4);
        for (int y = 0; y < sh; y++)
            for (int x = 0; x < sw; x++) {
                int i = (y * sw + x) * 4;
                src_data[i]   = (float)x / (float)sw;
                src_data[i+1] = (float)y / (float)sh;
                src_data[i+2] = 0.5f;
                src_data[i+3] = 1.0f;
            }
        ...
        // Create image2d for hardware bilinear (Adreno Texture Processor)
        cl_image_format img_fmt;
        img_fmt.image_channel_order = CL_BGRA;
        img_fmt.image_channel_data_type = CL_FLOAT;
        cl_image_desc img_desc = {};
        img_desc.image_type = CL_MEM_OBJECT_IMAGE2D;
        img_desc.image_width = sw;
        img_desc.image_height = sh;
        cl_mem src_image = clCreateImage(ctx, CL_MEM_READ_ONLY, &img_fmt, &img_desc, NULL, &cerr);
        CL_CHECK(cerr, "createImage src");
        ...
        // Upload source image to image2d (float RGBA data)
        size_t img_origin[3] = {0, 0, 0};
        size_t img_region[3] = {(size_t)sw, (size_t)sh, 1};
        clEnqueueWriteImage(queue, src_image, CL_TRUE, img_origin, img_region,
                            sw * 4 * (int)sizeof(float), 0, src_data.data(), 0, NULL, NULL);

        // Set kernel args (image2d_t version — no src_stride arg)
        clSetKernelArg(k_resize, 1, sizeof(cl_mem), &norm_buf);
        clSetKernelArg(k_resize, 2, sizeof(int), &sw);
        clSetKernelArg(k_resize, 3, sizeof(int), &sh);
        clSetKernelArg(k_resize, 4, sizeof(int), &DST_W);
        clSetKernelArg(k_resize, 5, sizeof(int), &DST_H);
        clSetKernelArg(k_resize, 6, sizeof(float), &MEAN_R);
        clSetKernelArg(k_resize, 7, sizeof(float), &MEAN_G);
        clSetKernelArg(k_resize, 8, sizeof(float), &MEAN_B);
        clSetKernelArg(k_resize, 9, sizeof(float), &inv_sr);
        clSetKernelArg(k_resize, 10, sizeof(float), &inv_sg);
        clSetKernelArg(k_resize, 11, sizeof(float), &inv_sb);
        ...
        // Warmup iterations
        for (int w = 0; w < 10; w++) {
            clSetKernelArg(k_resize, 0, sizeof(cl_mem), &src_image);
            clSetKernelArg(k_resize, 1, sizeof(cl_mem), &norm_buf);
        ...
        for (int iter = 0; iter < ITERS; iter++) {
            ...
            // Upload src to image2d (H2D transfer, float data)
            clEnqueueWriteImage(queue, src_image, CL_FALSE, img_origin, img_region,
                                sw * 4 * (int)sizeof(float), 0, src_data.data(), 0, NULL, &upload_ev);
        ...
        if (check_correctness) {
            ...
            cpuBaseline(src_data, sw, sh, cpu_dst, DST_W, DST_H);
        ...
        clReleaseMemObject(src_image); clReleaseMemObject(norm_buf);
